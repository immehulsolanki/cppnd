...
// directional deltas, to enable travel +x,+y and -x,-y.
...
void ExpandNeighbors(const vector<int> &current, int goal[2], vector<vector<int>> &openlist, vector<vector<State>> &grid){
  // TODO: Get current node's data.
  int x = current[0]; // extract data from current node 1d vector
  int y = current[1];
  int g = current[2];   
  // not h value, will we compute after extreacting nextnode.

  // TODO: Loop through current node's potential neighbors.
  for (int i = 0; i < 4; i++) {
    int x2 = x + delta[i][0]; // delta will add + and - to x and y direction of grid
    int y2 = y + delta[i][1]; // this will allow use to travel in all 4 directions.

    // TODO: Check that the potential neighbor's x2 and y2 values are on the grid and not closed.
    if (CheckValidCell(x2, y2, grid)) { // we will do this 4 time to find the valid node
      //valid means not zero and not empty and not obstacle           
      // TODO: Increment g value, compute h value, and add neighbor to open list.
      int g2 = g + 1; // increase g in accending order up to goal.
      int h2 = Heuristic(x2, y2, goal[0], goal[1]); // will get manhatten distance from current node
      // to goal so that we can sort it in open list.
      AddToOpen(x2, y2, g2, h2, openlist, grid);
      //add new valid node to openlist and mark current xy closed.
    }    

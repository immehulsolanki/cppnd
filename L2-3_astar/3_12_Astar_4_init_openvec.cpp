...
vector<vector<State>> Search(vector<vector<State>> grid, int init[2], int goal[2]) {
  ...
  vector<vector<int>> open {}; // init. blank 2d vector to send in addtoopen().
  // open vector will have 4 int values so we also need initial values.
  
  // TODO: Initialize the starting node. and to add in openlist 2d vector.
  int x = init[0]; //start point of grid x,y is from main(), mostly (0,0). 
  int y = init[1];
  int g = 0; // init. cost with 0, and will increase in loop accending order.
  int h = Heuristic(x, y, goal[0], goal[1]);  // heuristic manhattan Distance (0,0)(max,max)
  // heuristic is {x1, y1, x2, y2} so we send inial data as start point and goal.
  // initial H will be highest possible value and then decrese with going towards goal.
  // TODO: Use AddToOpen to add the starting node to the open vector.
  AddToOpen (x, y, g, h, open, grid); // now call with init. values.
  // to add {x,y,g.h} values to open list and mark grid as closed.

...
The last part of the A* algorithm to be implemented is the part 
that adds neighboring nodes to the open vector. 
In order to expand your A* search from the current node to neighboring nodes, 
you first will need to check that neighboring grid cells are not closed, 
and that they are not an obstacle. In this exercise, you will write a function CheckValidCell 
that does exactly this.

This Exercise:
    Write a function bool CheckValidCell that accepts two ints for the x and y coordinates 
    and a reference to the grid. The function should do two things:
...
bool CheckValidCell(int x, int y, vector<vector<State>> &grid){
  // two int x,y coordinate and grid reference.
  bool on_gridx = (x >= 0 && x < grid.size()); // if x isnot 0 and inside grid
  bool on_gridy = (y >= 0 && y < grid[0].size()); // if y is not 0 and inside grid
  // by default vector.size() will return size of rows, to get colomn size use index vector[0].size().
  
  // State::kEmpty ensures its not obstacle.
  if ((on_gridx && on_gridy) && (grid[x][y] == State::kEmpty)){
    // if both x and y are true as wellas state empty is true 
    // end logic will return true only if both conditions are fulfilled.
      return true;    
  }
  return false;
}
